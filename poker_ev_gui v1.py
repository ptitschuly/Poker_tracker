...
# --- REMPLACER evaluate_seven_card_hand ET calculate_equity ---

evaluator = Evaluator() # Créez une instance de l'évaluateur une seule fois

def calculate_equity_fast(hero_hand, opponent_range, community_cards, num_simulations=5000):
    """
...
"""
# def calculate_equity(hero_hand, opponent_range, community_cards, num_simulations=3000):
    
    # Calculates the hero's equity against an opponent's range using Monte Carlo simulation
    # for much faster performance. The precision can be tuned with num_simulations.
    
    if not opponent_range:
        return 1.0 # If opponent has no possible hands, we win by default
    ...
    # Filter opponent range based on known cards
    valid_opponent_range = {
        opp_hand for opp_hand in opponent_range 
        if not known_cards.intersection(set(opp_hand.cards))
    }

    if not valid_opponent_range:
        return 1.0 # Card removal makes opponent's range impossible

    num_cards_to_draw = 5 - len(community_cards)
    
    # Distribute the total simulations across the opponent's possible hands
    # This ensures a fair sampling across the entire range.
    sims_per_hand = max(1, num_simulations // len(valid_opponent_range))
    total_simulations = 0

    for opp_hand in valid_opponent_range:
        # Create a deck for this specific matchup
        matchup_deck = list(deck - set(opp_hand.cards))
        
        if len(matchup_deck) < num_cards_to_draw:
            continue

        for _ in range(sims_per_hand):
            total_simulations += 1
            
            # Randomly sample a runout instead of iterating through all combinations
            runout = random.sample(matchup_deck, num_cards_to_draw)
            
            board = community_cards + list(runout)
            
            hero_full_hand = list(hero_hand.cards) + board
            opp_full_hand = list(opp_hand.cards) + board

            hero_best = evaluate_seven_card_hand(hero_full_hand)
            opp_best = evaluate_seven_card_hand(opp_full_hand)
    ...
    if total_simulations == 0:
        # This can happen if the range is very wide and sims_per_hand is < 1
        # Fallback to a simpler simulation if needed, or return best guess.
        return 1.0 
...
def find_optimal_bet_from_gui(gui_elements):
    ...
    try:
        # --- This section is copied and adapted from calculate_ev_from_gui ---
        # It sets up the scenario just like the other function.
        ...
        # 1. Calculate EV of Folding
        ev_fold = 0.0

        # 2. Calculate EV of Calling
        # The amount to call is the big blind minus what hero already posted (e.g., the small blind)
        # For simplicity here, we'll assume the call size is the big blind if we are SB, or 0 if we are BB vs a limp.
        # A more robust implementation would track bets in a round.
        call_size = big_blind - (small_blind if selected_pos == 'SB' else 0)
        ev_call = calculate_chip_ev(scenario, player_name, opponent_range_string, 'call', call_size)

        # 3. Calculate EV for a range of Raise Sizes
        # Define bet sizes as a percentage of the pot.
        bet_size_multipliers = [0.33, 0.50, 0.75, 1.0, 1.5] 
        best_raise_ev = -float('inf')
        optimal_raise_size = 0

        effective_stack = min(players[0].stack, players[1].stack) # Assumes 1 opponent

        for multiplier in bet_size_multipliers:
            # A standard preflop raise is to a certain amount, not just adding on top.
            # Let's model a raise to X times the big blind. Common sizes are 2.5x, 3x, 3.5x
            # For post-flop, pot-percentage is more common. Let's stick to that for simplicity.
            raise_amount = min(scenario.pot * multiplier, effective_stack)
            
            if raise_amount <= call_size: continue # Not a valid raise
        ...
        # 4. Compare all options and find the best one
        best_action = "Fold"
        max_ev = ev_fold
        ...
        if best_action == "Raise":
            result_text = f"Optimal Action: Raise to {optimal_raise_size:.2f} (EV: {max_ev:.2f})"
        elif best_action == "Call":
            result_text = f"Optimal Action: Call {call_size:.2f} (EV: {max_ev:.2f})"
        else:
            result_text = f"Optimal Action: Fold (EV: {max_ev:.2f})"
...
def calculate_ev_from_gui(gui_elements):
    ...
    try:
        ...
        # Dynamically update position combobox based on number of players
        if n_players == 2:
            positions = ['SB', 'BB']
        else: # n_players == 3
            positions = ['BTN', 'SB', 'BB']
        ...
        for i, s in enumerate(stack_strings):
            stack = float(s)
            if stack < 0:
                raise ValueError(f"Player stack cannot be negative.")
            player_stacks_raw.append(stack)

        # Reorder stacks so the hero is always first
        hero_stack = player_stacks_raw[hero_pos_index]
        opponent_stacks = [s for i, s in enumerate(player_stacks_raw) if i != hero_pos_index]
        player_stacks = [hero_stack] + opponent_stacks

        # Create player names and objects with hero as Player 1
        player_names = [f"Player {i+1}" for i in range(n_players)]
        player_name = player_names[0] # Hero is always Player 1
        players = [Player(name, stack) for name, stack in zip(player_names, player_stacks)]
